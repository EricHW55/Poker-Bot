    ...
    def __call__(self, num:int, community_card:list, my_hand:list, opponent_hand:list, opponent_action_num:int, pot_size:int=0, loss:int=0, shape_to_num:bool=False) -> int: # 시뮬레이션 결과 반환
        ...
        for n,action in enumerate([0,1]): # action
            for i in range(num):
                ...
                # results[num2-2] = 0
                # num3 = results.index(max(results)) + 2
                if num1 > 14: num1 -= 13
                if num2 > 14: num2 -= 13
                # if num3 > 14: num3 -= 13
                error1 = random.choice([-1,0,1]); error2 = random.choice([-1,0,1])#; error3 = random.choice([-1,0,1])
                if num1+error1 == 1 or num1+error1 == 15: error1 = 0
                if num2+error2 == 1 or num2+error2 == 15: error2 = 0
                # if num3+error3 == 1 or num3+error3 == 15: error3 = 0
                if shape_to_num :
                    shape1 = random.choice([0, 1, 2, 3])
                    shape2 = random.choice([0, 1, 2, 3])
                    # shape3 = random.choice([0, 1, 2, 3])
                else:
                    shape1 = random.choice(['♠', '♡', '♢', '♣'])
                    shape2 = random.choice(['♠', '♡', '♢', '♣'])
                    # shape3 = random.choice(['♠', '♡', '♢', '♣'])
                ...
                for _index,p in enumerate(self.opponent_hand):
                    if tuple(p) in self.community_card + self.my_hand:
                        # print('P : ',p)
                        # self.opponent_hand[self.opponent_hand.index(p)] = [shape3, num3+error3]
                        # print(p)
                        # print(self.opponent_hand, self.community_card, self.my_hand)
                        if shape_to_num:
                            p[0] += 1
                            if p[0] == 4: p[0] = 0
                        else :
                ...
                # print(self.community_card, self.my_hand, self.opponent_hand)
